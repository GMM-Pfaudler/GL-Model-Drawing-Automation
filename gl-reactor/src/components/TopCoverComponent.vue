  ...
  <div class="q-pa-md" style="height: 100vh; display: flex; flex-direction: column;">
    ...
    <div class="q-mb-md">
      <div class="row q-gutter-md q-wrap">
        ...
        />
        <q-space/>
        <q-btn outline rounded icon="search" color="primary" label="Search" @click="searchTopCoverData"/>
      </div>
    </div>
...
export default {
  ...
  setup (props, { emit }) {
    ...
    const gasketMaterialOptions = ref(['-', 'MS', 'SS304', 'SS316'])
    ...
    const thermowellTipTypeOptions = ref([])
    ...
    const searchTopCoverData = () => {
        const topCoverData = prepareTopCoverData()
        emit('search-data', topCoverData)
    }

    const prepareTopCoverData = () => {
        ...
        for (let i = 0; i < nozzle_names.length; i++) {
            if(nozzle_names[i] !== 'L'){
                nozzles.value.push({
                    ...
                    radius: '-',
                    fittings: []
                    // statndardFittings.value[i]
                });
            }
        }
    }

    const fillTopCoverData = (data) => {
